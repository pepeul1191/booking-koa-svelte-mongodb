...
const getRooms = async ({
  ...
  try {
    ...
    if (all){
      // Obtener solo _id y name, ordenados por nombre
      const rooms = await Room.find(query)
        .select('_id name') // Solo seleccionar _id y name
        .sort({ name: 1 }); // Ordenar ascendentemente por nombre

      // Formatear _id como string
      const formattedRooms = rooms.map(room => ({
        _id: room._id.toString(), // Convertir ObjectId a string
        name: room.name
      }));
      ...
      return {
        success: true,
        data: formattedRooms,
        message: "Lista de salas obtenida exitosamente (solo IDs y nombres)",
      ...
      paginatedRooms = await Room.find(query)
        .sort(sortOptions)
        .skip(offset)
        .limit(per_page)
